document.addEventListener('DOMContentLoaded', function() {
    ...
    const initMVSlider = function() {
        const mvContainer = document.querySelector('.top-mv__img');
        const mvItems = document.querySelectorAll('.top-mv__item');
    ...
    const swiper = new Swiper('#slide02', {
        slidesPerView: 'auto',
        centeredSlides: true,
        loop: true,
        spaceBetween: 30,
    ...
    const linkEffect = new LinkHoverEffect();

    // Initialize anime elements
    initAnimeElem();
});
...
class LinkHoverEffect {
    constructor() {
        this.label = ".js-link-text__label";
        this.onInit();
    }

    onInit() {
        const linkTextItems = document.querySelectorAll(".js-link-text");
        if (linkTextItems) {
            linkTextItems.forEach((item) => {
                item.addEventListener("mouseenter", () => {

                    item.classList.add("is-anim");

                    const labelEl = item.querySelectorAll(this.label);
                    const hoverEl = item.querySelectorAll(".js-link-text__hover");

                    if (labelEl) {
                        gsap.fromTo(labelEl, { y: "0" }, { y: "-100%", duration: 0.4 });
                    }

                    if (hoverEl) {
                        gsap.fromTo(hoverEl, { y: "10%", delay: 0.2 }, { y: "-100%", duration: 0.6 });
                    }

                });

                item.addEventListener("mouseleave", () => {
                    item.classList.remove("is-anim");
                });
            });
        }
    }
    ...
    onDestroy() {
        // Chưa có xử lý hủy, có thể thêm sau nếu cần
    }
}
...
// Animation elements functionality
const initAnimeElem = function() {
    const animeElements = document.querySelectorAll('.js-anime-elem');
    ...
    const observeNewElements = function() {
        const newElements = document.querySelectorAll('.js-anime-elem:not(.is-animated)');
    ...
    // Expose function for dynamic content if needed
    window.initAnimeElemNewElements = observeNewElements;
};